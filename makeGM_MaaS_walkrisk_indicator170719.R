...
#'As a simple assumption I excluded any areas which are in ACORN categories 4 and 5.
...
#' If carrying out an social and distributional impact assessment the following 
#' might be useful:
...
count_car <- afford_MaaS %>% filter(Miles_pp < 2.6557)
count_car <- sum(count_car$caru5,na.rm = T)#7213 if no filtering 

df_count_commuteu5 <- data.frame( mode = c("walk","bike","bus","metro","rail","car"),
people =  c(count_walk,count_bike,count_bus,count_metro,count_rail,5900)
)

#make a barplot 
p <-ggplot(data=df_count_commuteu5, aes(x=mode, y=people)) +
  geom_bar(stat="identity",fill=c("grey","grey","purple","grey","grey","blue"))+
...
save_tmap(map_c, filename="walk_riskBD_170719.png")
...
map_c
save_tmap(map_c, filename="car_opportunityselectedbelowAvgMilesppBD_170719.png")
